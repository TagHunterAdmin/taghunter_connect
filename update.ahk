...
updateReady() {
	...
	if (req.status == 200 and StrLen(req.responseText) <= 64) { ; OK.
		...
		if (version != req.responseText) {
					result := MsgBox("Une nouvelle version de TagHunter Connect est disponible: " . req.responseText . ", mettre à jour?", "Nouvelle version", 0x2024)
					if result = "Yes"
					{
						try {
